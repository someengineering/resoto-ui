...
func handle_bad_response_codes(_response_code:int):
	match _response_code:
		401:
			if JWT.token_expired():
				_g.emit_signal("add_toast", "Your session has expired!", "[b]Reload the page to login again.[/b]", 1, self, -1, false)
				_g.authorized = false
#		400:
...
func post_graph_search(body:String, type:String="graph", 
	...
	var request = req_post("/graph/"+ graph +"/search/"+ type + "?section=%s&count=true" % section, body, accept_json_headers)
	request.connect("pre_done", self, "_transform_json")
	return request
