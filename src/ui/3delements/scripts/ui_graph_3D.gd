...
func generate_graph(filtered_data_result: Dictionary):
	...
	cam_tween.interpolate_method(
		self, "change_cam_zoom", graph_cam.fov, 30, 0.5, Tween.TRANS_QUART, Tween.EASE_IN_OUT
	)
	cam_tween.start()
...
func save_order(_saved_node_positions):
	...
	var flytime = range_lerp(
		clamp(target_pos.distance_to(graph_cam_arm.global_transform.origin), 100, 1000),
		100,
		1000,
		0.35,
		1.5
	...
	cam_tween.interpolate_method(
		self, "change_cam_zoom", graph_cam.fov, 90, flytime, Tween.TRANS_QUART, Tween.EASE_IN_OUT
	)
	cam_tween.interpolate_property(
		graph_cam_arm,
		"global_transform:origin",
		graph_cam_arm.global_transform.origin,
		target_pos,
		flytime,
		Tween.TRANS_QUART,
		Tween.EASE_IN_OUT
...
func _input(event):
	if event is InputEventPanGesture:
		var zoom_value = clamp(
			graph_cam.zoom.x + (-event.delta.y * TOUCH_ZOOM_SPEED), MAX_ZOOM_3D, MIN_ZOOM_3D
		)
...
func zoom_out():
	...
	cam_tween.remove_all()
	cam_tween.interpolate_method(
		self, "change_cam_zoom", graph_cam.fov, 40, 0.7, Tween.TRANS_EXPO, Tween.EASE_OUT
	)
	cam_tween.start()
...
func zoom_in():
	cam_tween.remove_all()
	cam_tween.interpolate_method(
		self, "change_cam_zoom", graph_cam.fov, original_zoom, 0.7, Tween.TRANS_EXPO, Tween.EASE_OUT
	)
	cam_tween.start()
...
func go_to_graph_node_3d(node_id, _graph) -> void:
	...
	cam_tween.remove_all()
	cam_tween.interpolate_method(
		self, "change_cam_zoom", graph_cam.fov, 15, flytime, Tween.TRANS_QUART, Tween.EASE_IN_OUT
	)
	cam_tween.interpolate_property(
		graph_cam_arm,
		"global_transform:origin",
...
func hide_info(_target_node):
	...
	cam_moving = false
	cam_tween.interpolate_method(
		self, "change_cam_zoom", graph_cam.fov, 30, 0.7, Tween.TRANS_EXPO, Tween.EASE_OUT
	)
	cam_tween.start()
