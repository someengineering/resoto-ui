...
func layout_blastradius() -> void:
	...
	for dist in sort_dict.values():
		...
		for node in dist:
			...
			var new_position = Vector2((blastradius_diameter / largest_dist) * node_dist, 0).rotated(
				PI + PI / 2 + ((TAU / nodes_on_ring) * i)
			)
	...
	tween.interpolate_method(
		self, "update_lines", 0, 1, float(largest_dist) * 0.2, Tween.TRANS_QUINT, Tween.EASE_OUT
	)
...
func draw_blastradius(center, radius, is_green := false) -> void:
	...
	for i in largest_dist + 1:
		...
		for x in resolution + 1:
			...
			new_points.append(
				center + Vector2(cos(angle_point), sin(angle_point)) * ((radius / largest_dist) * i)
			)
...
func clear_blastradius() -> void:
	...
	tween.interpolate_property(
		...
		Vector2.ONE,
		Vector2.ZERO,
		time_to_collapse * 1.5,
		Tween.TRANS_QUAD,
		Tween.EASE_OUT
	)
	tween.interpolate_method(
		self, "update_lines", 1, 0, time_to_collapse, Tween.TRANS_QUAD, Tween.EASE_OUT
	)
	for node in graph_view.graph_data.nodes.values():
		tween.interpolate_property(
			node.scene,
