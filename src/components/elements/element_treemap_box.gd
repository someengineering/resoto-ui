...
func _ready():
	...
	var font_color = Style.col_map[Style.c.BG2] if element_color.get_luminance() > 0.46 else Style.col_map[Style.c.LIGHT]
...
func update_label_pos():
	...
	if final_size.x < 30 or final_size.y < 30:
		...
		text_label.rect_size.x = final_size.x - 2
		text_label.rect_size.y = final_size.y - 2
		text_label.rect_size /= 0.5
		text_label.rect_position = -text_label.rect_size / 2
		scaler.scale = Vector2.ONE*0.5
	elif final_size.x < 150 or final_size.y < 120:
		text_label.rect_size.x = final_size.x - 7
		text_label.rect_size.y = final_size.y - 7
		text_label.rect_size /= 0.7
		text_label.rect_position = -text_label.rect_size / 2
