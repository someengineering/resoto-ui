...
func start() -> void:
	...
	loading = true
	tween.remove_all()
	tween.interpolate_property(self, "modulate:a", modulate.a, 1.0, 0.2, Tween.TRANS_EXPO, Tween.EASE_OUT, 0.2)
...
func stop() -> void:
	loading = false
	tween.remove_all()
	tween.interpolate_property(self, "modulate:a", modulate.a, 0.0, 0.8, Tween.TRANS_EXPO, Tween.EASE_OUT)
